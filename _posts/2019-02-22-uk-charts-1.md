...
Let us denote the set of unique labels in year _i_ by $\mathcal{L}_{i}$. The set of new labels, $\mathcal{S}^{N}_{i}$  is defined as:
...
The number of new, $\left| \mathcal{S}^{N}_{i} \right|$,  and departing labels,  $\left| \mathcal{S}^{D}_{i} \right|$ are easily obtained by a groupby operation on the `first` and `last` columns. We can then proeed to calculate the fraction of newborn
